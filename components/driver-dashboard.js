...
export default function DriverDashboard() {
    ...
    useEffect(() => {
        loadStats();
        loadRecentTrips();
    }, [loadStats, loadRecentTrips]);

    // Poll for incoming ride requests every 5s, only while online
    useEffect(() => {
        if (!online) {
    ...
    async function handleRideAction(action) {
        ...
        try {
            ...
            setIncomingRide(null);
            // Refresh stats + trips since accepting spins up a new Ongoing trip
            loadStats();
            loadRecentTrips();
        } catch (err) {
            setError(err.message);
    ...
    const statCards = stats
        ? [
            { label: "Today's Earnings", value: `₹${stats.todaysEarnings.toLocaleString()}`, icon: Wallet },
            { label: "Today's Trips", value: String(stats.todaysTripCount), icon: Car },
            { label: "Rating", value: stats.rating?.toFixed(2) || "—", icon: Star },
            { label: "Total Trips (all time)", value: String(stats.totalTrips), icon: ClipboardList },
        ]
        : [];
        ...
        <>
            ...
            <div className="flex justify-end -mt-2 sm:-mt-4">
                <button
                    ...
                    className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-colors duration-300 ${online ? "bg-green-50 text-green-600" : "bg-slate-100 text-slate-500"
                        }`}
                >
                    <span
                        className={`h-2 w-2 rounded-full ${online ? "bg-green-500 animate-pulse" : "bg-slate-400"
                            }`}
                    />
                        ...
                        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 font-bold text-lg">{incomingRide.riderName}</p>
                            ...
                            <div className="flex items-center gap-3">
                                <div className="text-right mr-1 hidden sm:block">
                                    <p className="text-2xl font-bold">₹{incomingRide.estimatedFare}</p>
                                    <p className="text-xs text-blue-100">Est. fare</p>
                                </div>
                                <button
                                    disabled={rideActionLoading}
                                    onClick={() => handleRideAction("decline")}
                                    className="rounded-full bg-white/15 hover:bg-white/25 px-5 py-2.5 text-sm font-semibold transition-colors disabled:opacity-50"
                                >
                                    Decline
                                </button>
                                <button
                                    ...
                                    className="rounded-full bg-white text-blue-700 hover:bg-blue-50 px-5 py-2.5 text-sm font-semibold transition-colors shadow-lg disabled:opacity-50"
                                >
                                    Accept
                                </button>
                            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                {statsLoading
                    ? Array.from({ length: 4 }).map((_, i) => (
                        <div
                            key={i}
                            className="rounded-2xl bg-white border border-slate-200 p-4 sm:p-5 shadow-sm animate-pulse h-32"
                        />
                    ))
                ...
                >
                    ...
                    <div className="mt-6 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 p-4 text-white">
                        ...
                        <p className="text-xs text-blue-100 mt-1">
                            ₹{stats?.totalEarnings?.toLocaleString() || 0} earned all-time.
                        </p>
                    </div>
                </motion.div>
            </div>
            ...
            >
                <div className="flex items-center justify-between px-5 sm:px-7 py-5 border-b border-slate-100">
                    ...
                    <a href="/my-trips" className="flex items-center gap-1 text-sm font-semibold text-blue-600 hover:text-blue-700">
                        View all
                ...
                {tripsLoading ? (
                    ...
                    <p className="p-8 text-center text-sm text-slate-400">No trips yet.</p>
                ) : (
                    <>
                            ...
                            {recentTrips.map((trip) => (
                                <div key={trip._id} className="p-4">
                                    <div className="flex items-center justify-between">
                                        <p className="font-semibold text-slate-900 text-sm">{trip.riderName}</p>
